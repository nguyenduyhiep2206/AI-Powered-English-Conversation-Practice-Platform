...
function retrievalEmptyNote(
  ...
  if (route === "retrieval_empty") {
    return "Nothing found in the books linked to this skill.";
  }
  return null;
}
...
export default function LessonQaPanel({
  ...
  const [open, setOpen] = useState(isDock || Boolean(demo?.defaultOpen));
  ...
  useEffect(() => {
    if (!panelOpen || loadedRef.current || isDemo) return;
    let cancelled = false;
  ...
  }, [panelOpen, skillId, isDemo]);

  useEffect(() => {
    if (!panelOpen) return;
    bottomRef.current?.scrollIntoView({ behavior: "smooth" });
  }, [panelOpen, messages, streamingText]);
  ...
  if (!panelOpen) {
    // Dock is always open — never fall through to FAB.
    if (isDock) {
      return null;
    }
    return (
